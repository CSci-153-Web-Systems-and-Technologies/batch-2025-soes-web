...
import CandidateRowActions from "../../_components/CandidateRowActions";
...
export default async function CandidatesPage({
  ...
  const { data, error: candidatesError } = await supabase
    .from("candidates")
    .select("id, student_id, full_name, description, avatar_url, positions(id, title)")
    .eq("election_id", id)
    .order("title", { foreignTable: "positions", ascending: true })
    .order("full_name", { ascending: true });

  if (candidatesError) {
    console.error("Error fetching candidates:", JSON.stringify(candidatesError, null, 2));
  }
